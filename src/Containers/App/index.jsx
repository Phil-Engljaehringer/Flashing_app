...
class App extends Component {
  ...
  constructor() {
    ...
    // Auto-use backend when the app is served from the companion (non-localhost).
    // The backend WebSocket lives at the same host:port as the HTTP server.
    const storedBackend = loadBackendUrl();
    const autoBackend =
      !storedBackend &&
      ...
      window.location.hostname !== '127.0.0.1'
        ...
        : null;
    this.backendUrl = storedBackend || autoBackend;
  ...
  async componentDidMount() {
    this.onMount(async() => {
      ...
      // Backend mode: all serial logic runs on the companion computer
      if (this.backendUrl) {
        this.backend = new BackendClient(this.backendUrl);
        ...
        try {
          ...
          const portNames = ports.map((p) => p.path || `${p.usbVendorId}:${p.usbProductId}`);
          store.dispatch(setChecked(true));
          store.dispatch(setHasSerial(true));
          store.dispatch(setConnected(portNames.length > 0));
          store.dispatch(setPortNames(portNames));
        } catch (e) {
          console.debug('Backend connection failed:', e.message);
          store.dispatch(setChecked(true));
        }
        return;
      }
  ...
  // ---------------------------------------------------------------------------
  // Backend mode — event handlers wired to the Node.js companion server
  // ---------------------------------------------------------------------------
  ...
  setupBackendHandlers() {
    ...
    b.on('log', (ev) => {
      this.addLogMessage(ev.message, ev.params);
    });

    b.on('connected', (ev) => {
      this.addLogMessage('mspFcInfo', {
        id: ev.fc.variant,
        version: ev.fc.version,
      });
      store.dispatch(setMspFeatures(ev.features));
      store.dispatch(setOpen(true));
      store.dispatch(setConnectedEscs(ev.motorCount));
    ...
    b.on('disconnected', () => {
      ...
      store.dispatch(setDisconnecting(false));
      this.addLogMessage('closedPort');
    });

    b.on('stateChange', (state) => {
      if (state.isReading !== undefined) store.dispatch(setReading(state.isReading));
      if (state.isWriting !== undefined) store.dispatch(setWriting(state.isWriting));
      if (state.isFlashing !== undefined) store.dispatch(setFlashing(state.isFlashing));
    });

    b.on('escsReady', (ev) => {
      store.dispatch(setFourWay(ev.fourWay));
      store.dispatch(setConnectedEscs(ev.connected));
      store.dispatch(setIndividual(ev.individual));
      store.dispatch(setMaster(ev.master));

      this.progressReferences = [];
      for (let i = 0; i < ev.connected; i += 1) {
        this.progressReferences[i] = React.createRef();
      }
    });

    b.on('settingsWritten', (ev) => {
      store.dispatch(setIndividual(ev.individual));
    });

    b.on('flashComplete', (ev) => {
      const { escs } = store.getState();
      const individualIndex = escs.individual.findIndex((e) => e.index === ev.target);
    ...
    b.on('progress', (ev) => {
      const ref = this.progressReferences[ev.target];
      if (ref && ref.current) {
        ref.current.setProgress(ev.percent);
      }
    });

    b.on('firmwareDump', (ev) => {
      const binary = atob(ev.data);
      const bytes = new Uint8Array(binary.length);
      for (let i = 0; i < binary.length; i += 1) bytes[i] = binary.charCodeAt(i);
      const binToHex = new BinToHex(16, 0x00, 0xFF);
      const dataHex = binToHex.convert(bytes);
    ...
    b.on('batteryState', () => {
      // Battery state events are not currently used by the frontend
    });

    b.on('error', (ev) => {
      console.debug(`[backend] error in ${ev.cmd}: ${ev.message}`);
      store.dispatch(setConnecting(false));
      store.dispatch(setDisconnecting(false));
    });
  ...
  // ---------------------------------------------------------------------------
  // Backend-aware public handlers (override serial-mode handlers when backend)
  // ---------------------------------------------------------------------------

  handleSetPort = async() => {
    if (this.backend) {
      // Refresh the port list from the backend
      try {
        const ports = await this.backend.getPorts();
        const portNames = ports.map((p) => p.path || `${p.usbVendorId}:${p.usbProductId}`);
        store.dispatch(setConnected(portNames.length > 0));
        ...
        console.debug('Failed to refresh port list:', e);
      }
      return;
    }

    // Original serial-mode behaviour
    try {
  ...
  handleChangePort = async(index) => {
    if (this.backend) {
      // In backend mode, the portNames list is from the backend
      const { serial: serialState } = store.getState();
      this._selectedBackendPort = serialState.portNames[index];
  ...
  handleConnect = async(e) => {
    ...
    if (this.backend) {
      const { serial: serialState } = store.getState();
      const port = this._selectedBackendPort || serialState.portNames[0];
      const direct = loadDirectMode();
      const baudRate = direct ? 19200 : (serialState.baudRate || 115200);
      this.backend.connectSerial(port, baudRate, direct);
      return;
    }
  ...
  handleDisconnect = async(e) => {
    ...
    if (this.backend) {
      this.backend.disconnectSerial();
      return; // rest of state changes happen via 'disconnected' event
    }
  ...
  handleResetDefaultls = async() => {
    ...
    if (this.backend) {
      const { escs } = store.getState();
      this.backend.resetDefaults(escs.individual);
      return;
    }
  ...
  handleWriteSettings = async() => {
    ...
    if (this.backend) {
      const { escs } = store.getState();
      this.backend.writeSettings(escs.individual, escs.master);
      return;
    }
  ...
  handleFirmwareDump = async (target) => {
    if (this.backend) {
      const { escs } = store.getState();
      this.backend.readFirmware(target, escs.individual);
      return;
    }
  ...
  handleLocalSubmit = (e, force, migrate) => {
    ...
    reader.onload = async (e) => {
      if (this.backend) {
        this.backend.flashText(e.target.result, targets, escs.individual, force, migrate);
        return;
      }
  ...
  handleFlashUrl = async(url, layout, name, version, pwm, force, migrate) => {
    ...
    if (this.backend) {
      this.backend.flashUrl(url, targets, escs.individual, force, migrate);
      return;
    }
  ...
  handleAllMotorSpeed = async(speed) => {
    if (this.backend) { this.backend.setAllMotorSpeed(speed); return; }
    await this.serial.spinAllMotors(speed);
  };

  handleSingleMotorSpeed = async(index, speed) => {
    if (this.backend) { this.backend.setMotorSpeed(index, speed); return; }
