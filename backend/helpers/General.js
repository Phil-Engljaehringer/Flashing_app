// helpers/General.js — ported for Node.js backend.
// React/Redux deps removed: getAppSetting() now reads from a plain config object.

import sources from '../sources/index.js';

// Config is set once at server startup via setConfig().
let _config = {
  extendedDebug: false,
  skipCache: false,
  disableCommon: false,
  printLogs: false,
};

function setConfig(cfg) {
  _config = { ..._config, ...cfg };
}

function getAppSetting(name) {
  return _config[name];
}

function compare(a, b) {
  if (a.byteLength !== b.byteLength) {
    return false;
  }
  for (var i = 0; i < a.byteLength; i += 1) {
    if (a[i] !== b[i]) {
      return false;
    }
  }
  return true;
}

async function delay(ms) {
  await new Promise((resolve) => setTimeout(resolve, ms));
}

function isValidFlash(mcu, flash) {
  const firstBytes = flash.subarray(0, 3);
  const ljmpReset = new Uint8Array([0x02, 0x19, 0xFD]);
  const ljmpResetBB51 = new Uint8Array([0x02, 0x2F, 0xFD]);
  const ljmpCheckBootload = new Uint8Array([0x02, 0x19, 0xE0]);

  return !(
    !(mcu.includes('#BLHELI#') || mcu.includes('#BLHELI$')) ||
    (
      !compare(firstBytes, ljmpReset) &&
      !compare(firstBytes, ljmpCheckBootload) &&
      !compare(firstBytes, ljmpResetBB51)
    )
  );
}

async function retry(func, maxRetries, iterationDelay = 0) {
  function wrapped() {
    return new Promise((resolve, reject) => func(resolve, reject));
  }

  const process = async (resolve, reject) => {
    let retries = 0;
    let done = false;
    let result = null;

    while (!done) {
      try {
        result = await wrapped();
        done = true;
      } catch (e) {
        retries += 1;
        if (retries >= maxRetries) {
          return reject(e);
        }
        if (iterationDelay) {
          await delay(iterationDelay * retries);
        }
      }
    }

    return resolve(result);
  };

  return new Promise((resolve, reject) => process(resolve, reject));
}

const findMCU = (signature, MCUList) =>
  MCUList.find((mcu) => parseInt(mcu.signature, 16) === signature);

const isValidLayout = (layout) =>
  sources.some((source) => source.isValidLayout(layout));

const getSupportedSources = (signature) =>
  sources.filter((source) => findMCU(signature, source.getMcus()));

const getPwm = (name, version) => {
  const source = sources.filter((source) => source.name === name);
  if (source.length > 0) {
    return source[0].getPwm(version);
  }
  return [];
};

const getSource = (name) => {
  const options = sources.filter((source) => source.getName() === name);
  if (options.length > 0) {
    return options[0];
  }
  return null;
};

export {
  retry,
  delay,
  compare,
  findMCU,
  isValidFlash,
  isValidLayout,
  getSupportedSources,
  getAppSetting,
  getPwm,
  getSource,
  setConfig,
};
